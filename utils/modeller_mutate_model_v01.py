...
from basic import fasta
from basic import structure
#
#  mutate_model.py
#
...
if __name__ == "__main__":
    ...
    try:
        refseq = sys.argv[3]
        refseq = list(fasta.fasta2dic(refseq).values())[0]
    except IndexError:
        print("\nWaning: Without refsequence, the output will be named without wildtype residue!\n")
        refseq = False
    with open(resfile) as resfile:
        for line in resfile:
            lst = line.strip().split(" ")
            respos = lst[0]
            restyp = lst[1]
            chain = lst[2]
            out_name = run_mutate(modelname, respos, restyp, chain)
            if refseq != False:
                wild = refseq[int(respos) - 1]
                newname = "_".join([modelname.replace(".pdb",""),wild,respos,structure._3_2_1(restyp),".pdb"])
